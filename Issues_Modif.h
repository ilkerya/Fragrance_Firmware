...
1 - The fan by default runs by full speed when no PWM applied, it even runs when 5V supply applied insted of 12V unfortunately, the speed is low
     ...
     when the uc goes into sleep mode the PWM cant be supplied anymore afterwards the fan begins to rotate becuse the dc-dc converter supplies
     voltage around 5V ''the fan might work even at 3V ??'. we need to cut the supply of the dc-dc converter fully to avoid this.
     A controlled ldo can be used, there are specific pins that the output  can be latched by the uc during sleep GPIO2 is suitable for that
