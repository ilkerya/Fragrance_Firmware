...
void setup() {
    #define ON 1// no
  #define OFF 0// no
    #define SENSOR_3V_ENABLE 23//IO23
  pinMode(SENSOR_3V_ENABLE, OUTPUT);  
  digitalWrite(SENSOR_3V_ENABLE, ON); 

  #define FAN_PWM  15 //SENSOR_VN
  pinMode(FAN_PWM, OUTPUT);  
  digitalWrite(FAN_PWM, OFF); 

  #define ENABLE_BOOST_CONV 2 //34 RTC_GPIO4 
  pinMode(ENABLE_BOOST_CONV, OUTPUT);  
  digitalWrite(ENABLE_BOOST_CONV, ON); 
  ...
  ledcAttach(ledR, 25000, 8);  // 12 kHz PWM, 8-bit resolution
  ledcAttach(ledG, 25000, 8);
  ledcAttach(ledB, 25000, 8);
